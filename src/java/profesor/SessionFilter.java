...
import util.security.CookiesCodes;
	...
	/*
        ...
        for (Cookie cookie: cookies) {
            if(cookie.getName().equals(CookiesCodes.CS)){
                ...
                exists = ru.tokenExist(cookie.getValue());
                //System.out.println(exists);
                
            }
        }
        //System.out.println((Boolean)ses.getAttribute("auth"));
        if(!exists) {
            wrappedResponse.sendRedirect("../index.jsp");
        }else if(!(Boolean)ses.getAttribute("auth")){
            wrappedResponse.sendRedirect("../index.jsp");
        }else{
            ReadUsuario rus = new ReadUsuario();
            String nombre = rus.getNombreUsuario((Integer)ses.getAttribute("id_usu")); 
            wrappedRequest.setAttribute("nombre", nombre);
            String grupos[] = {"5IV7", "5IV8", "3IV8"};
            wrappedRequest.setAttribute("grupos", grupos);
            //wrappedResponse.sendRedirect(URI);
            //ses.getServletContext().getRequestDispatcher(URI).include(request, response);
